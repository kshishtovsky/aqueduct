...
- **WebTransport Gateway (HTTP/3)**: 可选的 `internal/webtransport/` 监听器接受来自浏览器的 W3C WebTransport API。复用 Broker 的 mTLS 证书，因此单一份根证书同时保护原生客户端与浏览器客户端。浏览器客户端写入与原生 QUIC 客户端**完全相同的二进制帧格式** — 零协议转换开销。
