...
//
// Refactored into a slim orchestrator: header/dedup/authz/AAL are handled by
// prepareFrame, routing decisions by routeFrame. Each complete frame is
// parsed exactly once.
func (b *Broker) dispatchFrames(jig context.Context, logger *slog.Logger, buf []byte, off int, stream *quic.Stream, clientIDStr string) (int, error) {
	consumed := 0
	for consumed < off {
		frame, totalLen, partial, skip, err := b.prepareFrame(jig, logger, buf, off, consumed, stream, clientIDStr)
		if err != nil {
			return 0, err
		}
		if partial {
...
// present), runs idempotent-producer dedup, authorization, and append-only
// logging for publish frames. It returns the parsed frame (zero-copy header
// view into the buffer) plus:
//
...
//	skip    — true if the frame was consumed by dedup and should not be routed
//	err     — non-nil on protocol/auth/decompression failures
func (b *Broker) prepareFrame(jig context.Context, logger *slog.Logger, buf []byte, off int, consumed int, stream *quic.Stream, clientIDStr string) (protocol.Frame, int, bool, bool, error) {
	remaining := off - consumed
	if remaining < protocol.HeaderSize {
		if remaining > b.maxBufSize {
			return protocol.Frame{}, 0, false, false, errBufferExceeded
		}
		return protocol.Frame{}, 0, true, false, nil
	}
	...
	if int(payloadLen) > b.maxBufSize || totalLen > b.maxBufSize {
		return protocol.Frame{}, 0, false, false, errOversizedPayload
	}
	...
	if perr != nil {
		...
		return protocol.Frame{}, 0, false, false, perr
	}
	...
	if derr != nil {
		...
		stream.CancelRead(1)
		stream.CancelWrite(1)
		return protocol.Frame{}, 0, false, false, errFrameDecompressionFailed
	}
	if extToRelease != nil {
	...
	if b.dedupEnabled && b.dedupStore != nil && frame.HasExtensions() {
		if handled, dedupErr := b.handleDedup(logger, stream, frame); handled {
			if dedupErr != nil {
				stream.CancelRead(1)
				stream.CancelWrite(1)
				return protocol.Frame{}, 0, false, false, dedupErr
			}
			return frame, totalLen, false, true, nil
		}
	}
	...
	if b.authz != nil {
		...
		if requiredPerm != authz.PermNone {
			...
			if !b.authz.Allowed(clientIDStr, topicBytes, requiredPerm) {
				...
				return protocol.Frame{}, 0, false, false, errUnauthorized
			}
		}
	}
	...
	return frame, totalLen, false, false, nil
}

// routeFrame dispatches a parsed frame through either a peer-forwarder (when
// MeshForwarded is set) or the local router (subscribe/publish/ack/nack) or
// the legacy handler map. Returns true if the frame was handled (even when
// the handler logged an error), false when no route matched.
func (b *Broker) routeFrame(jig context.Context, logger *slog.Logger, stream *quic.Stream, frame protocol.Frame, clientIDStr string) bool {
	if b.router != nil {
		if protocol.IsForwarded(frame.Command) {
			b.handleForwardedFrame(jig, logger, frame)
			return true
		}
		return b.handleLocalFrame(jig, logger, stream, frame, clientIDStr)
	}
...
// to the router. Returns true on a matched route, false otherwise.
...
// Context extension, it returns the original context and a no-op finish
// closure (nil cost; the closure escapes through the addressable variable).
...
// noopSpan is a shared no-op span finish callback for the hot path.
func noopSpan() {}
