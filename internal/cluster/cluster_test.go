...
// testNode is a simplified Aqueduct node for integration testing with the real broker.Router.
...
// newTestNode creates a QUIC listener with a real broker.Router and a PeerManager.
// Accept loop handles subscribe frames (registers via router.Subscribe) and peer-forwarded
// publish frames (delivers via router.PublishFromPeer).
func newTestNode(ctx context.Context, sTLS *tls.Config, qConf *quic.Config, peerAddrs []string, cTLS *tls.Config) (*testNode, error) {
	ln, err := quic.ListenAddr("127.0.0.1:0", sTLS, qConf)
	if err != nil {
		return nil, err
	}
	pm := cluster.New(ctx, peerAddrs, cTLS, qConf)
...
// waitPeers blocks until all nodes have connected to the expected number of peers.
func waitPeers(t *testing.T, nodes []*testNode, timeout time.Duration) {
	t.Helper()
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		allConnected := true
		for _, n := range nodes {
			if n.pm.ActivePeers() < len(nodes)-1 {
				allConnected = false
				break
			}
		}
	...
	for _, n := range nodes {
		t.Logf("node %s: %d/%d peers connected", n.addr, n.pm.ActivePeers(), len(nodes)-1)
	}
}
