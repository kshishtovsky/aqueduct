...
func TestBuildIdempotentExtension(t *testing.T) {
	const producerID uint64 = 0xDEADBEEFCAFEBABE
	const seqNum uint64 = 42

	extBlock := BuildIdempotentExtension(producerID, seqNum)
	defer ReleaseExtensions(extBlock)
	...
	totalLen := ExtTotalLen(extBlock, 0)
	wantTotal := IdempotentEntrySize
	if totalLen != wantTotal {
		t.Fatalf("totalLen = %d, want %d", totalLen, wantTotal)
	}

	gotID, ok := ExtractProducerID(extBlock)
	if !ok {
		t.Fatal("producer ID not found")
	}
	if gotID != producerID {
		t.Fatalf("producerID = %d, want %d", gotID, producerID)
	}

	gotSeq, ok := ExtractSeqNum(extBlock)
	if !ok {
		t.Fatal("seq num not found")
	}
	if gotSeq != seqNum {
		t.Fatalf("seqNum = %d, want %d", gotSeq, seqNum)
	}
}

func TestExtractProducerIDNotFound(t *testing.T) {
	if _, ok := ExtractProducerID(nil); ok {
		t.Fatal("expected not ok for nil block")
	}

	extBlock := BuildIdempotentExtension(1, 1)
	defer ReleaseExtensions(extBlock)

	if _, ok := ExtractProducerID(extBlock[:ExtHeaderLen]); ok {
		t.Fatal("expected not ok for truncated block")
	}
}

func TestExtractSeqNumNotFound(t *testing.T) {
	if _, ok := ExtractSeqNum(nil); ok {
		t.Fatal("expected not ok for nil block")
	}

	// ProducerID-only block must not yield a seq num.
	// We build a block containing only a ProducerID TLV.
	onlyProducerID := make([]byte, ExtHeaderLen+ExtEntryHeader+ExtProducerIDLen)
	SetExtTotalLen(onlyProducerID, 0, ExtEntryHeader+ExtProducerIDLen)
	off := ExtHeaderLen
	off = SetProducerID(onlyProducerID, off, 123)

	if _, ok := ExtractSeqNum(onlyProducerID); ok {
		t.Fatal("expected not ok for seq-num extraction from ProducerID-only block")
	}
}

func TestIdempotentExtensionWithTraceContext(t *testing.T) {
	const producerID uint64 = 0x1122334455667788
	const seqNum uint64 = 100
	...
	merged := BuildIdempotentExtensionFromExisting(ctxBlock, producerID, seqNum)
	defer ReleaseExtensions(merged)

	// Trace context must still be extractable.
	gotTID, gotSID, gotFlags, ok := ExtractTraceContext(merged)
	if !ok {
		t.Fatal("trace context lost after merge with idempotent TLVs")
	}
	if !bytes.Equal(gotTID, traceID) || !bytes.Equal(gotSID, spanID) || gotFlags != 1 {
		t.Fatal("trace context payload mismatch after merge")
	}

	// ProducerID/SeqNum must be present and intact.
	id, ok := ExtractProducerID(merged)
	if !ok || id != producerID {
		t.Fatalf("producerID = (%d, %v), want (%d, true)", id, ok, producerID)
	}
	seq, ok := ExtractSeqNum(merged)
	if !ok || seq != seqNum {
		t.Fatalf("seqNum = (%d, %v), want (%d, true)", seq, ok, seqNum)
	}
}

func TestIdempotentExtensionFromNilExisting(t *testing.T) {
	// When existing is nil, the merged block contains only the idempotent pair.
	merged := BuildIdempotentExtensionFromExisting(nil, 7, 11)
	defer ReleaseExtensions(merged)

	if _, ok := ExtractProducerID(merged); !ok {
		t.Fatal("producerID missing in merged block from nil")
	}
	if _, ok := ExtractSeqNum(merged); !ok {
		t.Fatal("seqNum missing in merged block from nil")
	}
}

func TestIdempotentExtensionRoundTripInFrame(t *testing.T) {
	const producerID uint64 = 0xABCDEF0102030405
	const seqNum uint64 = 999999

	extBlock := BuildIdempotentExtension(producerID, seqNum)
	defer ReleaseExtensions(extBlock)

	payload := []byte("topic:orders")
	fp := SerializeFrameWithExtensions(CmdPublish, 0, extBlock, payload)
	defer ReleaseBuffer(fp)

	frame, err := ParseFrame(*fp)
	if err != nil {
		t.Fatalf("ParseFrame error: %v", err)
	}
	if !frame.HasExtensions() {
		t.Fatal("HasExtensions bit missing")
	}

	gotID, ok := ExtractProducerID(frame.Extensions)
	if !ok {
		t.Fatal("producerID missing after round-trip")
	}
	if gotID != producerID {
		t.Fatalf("producerID = %d, want %d", gotID, producerID)
	}

	gotSeq, ok := ExtractSeqNum(frame.Extensions)
	if !ok {
		t.Fatal("seqNum missing after round-trip")
	}
	if gotSeq != seqNum {
		t.Fatalf("seqNum = %d, want %d", gotSeq, seqNum)
	}
...
func BenchmarkExtractProducerID(b *testing.B) {
	extBlock := BuildIdempotentExtension(42, 7)
	defer ReleaseExtensions(extBlock)
	...
	for i := 0; i < b.N; i++ {
		_, ok := ExtractProducerID(extBlock)
		if !ok {
			b.Fatal("not found")
		}
	}
...
func BenchmarkBuildIdempotentExtension(b *testing.B) {
	b.ReportAllocs()
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		extBlock := BuildIdempotentExtension(uint64(i), uint64(i))
		ReleaseExtensions(extBlock)
	}
}
