...
- **WebTransport Gateway (HTTP/3)**: Опциональный листенер `internal/webtransport/` принимает W3C WebTransport API из браузеров. Переиспользует mTLS-сертификат брокера — единый trust root для нативных и браузерных клиентов. Браузеры пишут **тот же бинарный формат фреймов**, что и нативные QUIC-клиенты — нулевой overhead на трансляцию.
